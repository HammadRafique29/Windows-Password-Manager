...
def identify(session_handle, unit_id):
    ...
    ret = lib.WinBioIdentify(session_handle, ctypes.byref(unit_id), ctypes.byref(identity),
    ...
    print(f"Identity AccountSid Data\t: {list(identity.Value.AccountSid.Data)[0:identity.Value.AccountSid.Size]}")
...
def verify(session_handle, unit_id, subfactor, identity):
    ...
    ret = lib.WinBioVerify(session_handle, ctypes.byref(identity),
                           subfactor, ctypes.byref(subfactor),
...
def get_token_information(identity):
    ...
    res = GetTokenInformation(get_process_token(),
